...
module.exports = {
    ...
    invoke: (conversation, done) => {
        ...
        const pricePerUnit = Number(conversation.properties().pricePerUnit.trim());
        const orderQuantity = Number(conversation.properties().orderQuantity);
        const paymentDetails = conversation.properties().paymentDetails;
        const deliveryType = conversation.properties().deliveryType;
        const deliveryCost = Number(conversation.properties().deliveryCost);
        ...
        const discountPercent = Number(conversation.properties().discountPercent ? conversation.properties().discountPercent > 1 ?conversation.properties().discountPercent / 100 : conversation.properties().discountPercent : 0);
        ...
        if (locale == "de") {
            ...
            responses = [
                (firstName ? " Hey " + firstName + ". " : "") + "Merci d'avoir envoyé des fleurs avec 24hrsflowers. Laissez-moi résumer votre commande #" + orderId,
                "Sous peu de temps, 24hrsflowers livrera " + (orderQuantity > 1 ? (orderQuantity + " unités") : (orderQuantity + "")) + " de " + productName + " à " + recipientName + "," + recipientAddress,
                "Le numéro de mobile du destinataire est " + recipientMobile,
                "Un coût de " + ((orderQuantity * pricePerUnit) - (orderQuantity * pricePerUnit * discountPercent)) + "$ + " + deliveryCost + "$ pour une expédition " + deliveryType + " sera débité sur votre carte de crédit",
                "Détails de paiement: " + paymentDetails,
                (cardSenderName || cardMessage) ? "Vos fleurs seront livrées avec une carte de voeux" : "Vos fleurs seront livrées sans carte de voeux",
        ...
        var editLabel = locale == "de" ? "Bestellung ändern" : locale == "fr" ? "Modifier la commande" : "Change order";
        ...
        let message = locale == "de" ? "Wie möchtest Du fortfahren? " : locale == "fr" ? "Comment voulez-vous procéder?" : "How do you want to proceed?";
